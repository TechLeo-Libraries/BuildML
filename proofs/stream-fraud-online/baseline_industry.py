"""Tier C: sklearn SGDClassifier partial_fit twin for stream-fraud-online."""
...
def main() -> None:
    ...
    session = (
        ...
        .set_roles(
            {"amount_z": "feature", "velocity_z": "feature", "is_fraud": "target"}
        )
    ...
    x_train = scaler.fit_transform(frame.loc[train_idx, ["amount_z", "velocity_z"]])
    y_train = frame.loc[train_idx, "is_fraud"].to_numpy()
    x_test = scaler.transform(frame.loc[test_idx, ["amount_z", "velocity_z"]])
    y_test = frame.loc[test_idx, "is_fraud"].to_numpy()
    ...
    print("stream-fraud-online Tier C OK", industry_metrics)
