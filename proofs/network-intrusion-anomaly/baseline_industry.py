"""Tier C: sklearn IsolationForest twin for network-intrusion-anomaly."""
...
def main() -> None:
    ...
    frame, _ = load_intrusion_anomaly_synthetic(seed=ctx.seed)
    session = (
    ...
    # Fit on train only (unsupervised); tune decision threshold on validation.
    clf = IsolationForest(
        n_estimators=200,
        contamination=0.06,
    ...
    # Threshold: maximize F1 on validation labels (same policy as BuildML).
    ...
    print("network-intrusion-anomaly Tier C OK", industry_metrics)
