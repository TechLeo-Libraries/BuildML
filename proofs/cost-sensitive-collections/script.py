"""Tier A proof: cost-sensitive-collections."""
...
def main() -> None:
    ...
    x, y = make_classification(
        ...
        weights=[0.75, 0.25], random_state=ctx.seed,
    )
    ...
    session = (
        ...
        .set_roles({**{c: "feature" for c in frame.columns if c.startswith("f")}, "delinquent": "target"})
    ...
    thr = session.decision.fit(
        method="threshold", partition="validation", fp_cost=1.0, fn_cost=4.0,
    )
    eval_thr = session.decision.evaluate(partition="test")
    knap = session.decision.fit(
        method="knapsack", partition="validation", budget=60.0,
    ...
    write_results(ctx, {
        "status": "completed",
        "data": {"name": "synthetic_collections", "license": "synthetic/public-domain", "n_rows": int(len(frame))},
        ...
        "limitations": ["Not general OR; disclosed knapsack/threshold helpers only"],
    })
    print("cost-sensitive-collections OK", eval_thr.to_dict() if hasattr(eval_thr, "to_dict") else eval_thr)
