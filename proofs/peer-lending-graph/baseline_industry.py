"""Tier C: networkx degree features + sklearn twin for peer-lending-graph."""
...
def main() -> None:
    ...
    visible = set(nodes.loc[train_idx + val_idx, "borrower_id"])
    deg = {nid: 0 for nid in nodes["borrower_id"]}
    if extra_available("networkx"):
        import networkx as nx

        g = nx.Graph()
        g.add_nodes_from(nodes["borrower_id"])
        ...
        clustering = {nid: 0.0 for nid in nodes["borrower_id"]}
        for src, tgt in edge_frame.itertuples(index=False):
            if src in visible and tgt in visible:
                deg[src] += 1
                deg[tgt] += 1
    ...
    feat = nodes.copy()
    feat["degree"] = feat["borrower_id"].map(deg).astype(float)
    feat["clustering"] = feat["borrower_id"].map(clustering).astype(float)
    cols = ["credit_z", "income_z", "degree", "clustering"]

    scaler = StandardScaler()
    x_train = scaler.fit_transform(feat.loc[train_idx, cols])
    y_train = feat.loc[train_idx, "is_default_ring"].to_numpy()
    x_test = scaler.transform(feat.loc[test_idx, cols])
    y_test = feat.loc[test_idx, "is_default_ring"].to_numpy()
    ...
    write_comparison(
        ctx,
        buildml={
            "backend": "buildml.Session.fit_graph(classical)",
    ...
    print("peer-lending-graph Tier C OK", industry_metrics)
