...
| 15 | Optional: [Multi-task](quickstart-multi-task.md) → [Multi-task deep](multi-task-deep.md) | Multi-output / Chain → per-task eval → bundle |
...
| Anomaly / fraud | `fit_anomaly`, `score_anomalies`, `evaluate_anomaly`, anomaly bundle | [Anomaly quickstart](quickstart-anomaly.md), [Anomaly deep](anomaly-deep.md) |
...
| Synthetic data | `fit_synthesizer`, `sample_synthetic`, `evaluate_synthetic`, synthetic bundle | [Synthetic quickstart](quickstart-synthetic.md), [Synthetic deep](synthetic-deep.md) |
...
| [Anomaly / fraud](quickstart-anomaly.md) | core | IsolationForest/LOF/OCSVM + supervised; score/eval/bundle |
...
| [Multi-task / multi-output](quickstart-multi-task.md) | core | MultiOutput / Chain → per-task + aggregate eval → multitask bundle |
...
| [Graph ML](quickstart-graph.md) | `buildml[graph]` (+ `torch` for GCN) | Node classify: NetworkX classical features + pure-Torch GCN; ≠ KG / Neo4j |
| [Symbolic / neuro-symbolic](quickstart-symbolic.md) | core | Declared/tree/list rules → traces; sklearn hybrid → symbolic bundle |
| [Case-based reasoning](quickstart-cbr.md) | core | Train case memory → kNN retrieve/reuse → traces → CBR bundle (≠ RAG) |
| [Imitation + RL](quickstart-imitation-rl.md) | core (+ optional `rl`) | BC from demos; contextual bandit; optional Gymnasium REINFORCE |
...
| [Search / LTR](quickstart-ranking.md) | core | Query–item feature rows + relevance; pointwise / RankSVM-lite; ≠ RAG / recommenders |
...
| [Synthetic data](quickstart-synthetic.md) | core (`smote` → `imbalanced`) | Bootstrap / Gaussian copula / SMOTE; fidelity + TSTR; ≠ DP / resample |
...
| [Multi-task deep](multi-task-deep.md) | MultiOutput / Chain, same-type multi-target, per-task metrics, multitask bundles |
...
| [Graph deep](graph-deep.md) | Node classify: NetworkX classical features + pure-Torch GCN, inductive/transductive, graph bundles |
...
| [Imitation + RL deep](imitation-rl-deep.md) | BC, LinUCB/ε-greedy/softmax bandits, offline DM/IPS, optional Gymnasium REINFORCE |
...
| [LTR deep](ranking-deep.md) | Train-only tabular LTR, group_split queries, nDCG@K/MAP@K/MRR@K, pointwise/pairwise, bundles |
...
| [Synthetic deep](synthetic-deep.md) | Train-only bootstrap / copula / SMOTE, fidelity vs TSTR, merge provenance, privacy limits |
