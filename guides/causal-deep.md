...
1. `session.causal.declare_assumptions` records treatment, outcome, confounders,
   estimand=`ATE`, identification=`backdoor`, and requires
   `acknowledge_unconfoundedness=True` plus `acknowledge_positivity=True`.
2. `session.causal.fit` fits on Session **train only** and estimates ATE. Backends:
...
3. `session.causal.estimate` / `session.causal.evaluate` score a partition with **fixed** train
   models. Holdout metrics check predictive quality: they do **not** prove
   identification.
4. `session.causal.refute` runs sensitivity checks. Native: placebo / random confounder.
   DoWhy: full refutation suite (placebo, random common cause, unobserved
   confounder, data subset, placebo outcome).
5. `session.causal.save_bundle` / `session.causal.load_bundle` persist `CausalPlan` separately
...
- Bootstrap in `session.causal.fit` resamples **train** (native/econml).
- Partition bootstrap in `session.causal.estimate` keeps nuisances fixed (native).
...
Known limits (accepted): no IV/front-door; binary treatment; ATE not full CATE
product (heterogeneity via causal_forest only).
