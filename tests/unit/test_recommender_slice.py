...
def test_fit_requires_split() -> None:
    ...
    with pytest.raises((LeakageError, ValidationError)):
        session.recommender.fit(user_column="user_id", item_column="item_id")
...
def test_session_fit_recommend_evaluate_bundle(tmp_path: Path) -> None:
    ...
    session.recommender.save_bundle(tmp_path / "rec")
    other = _demo_session()
    other.recommender.load_bundle(tmp_path / "rec", trusted=True)
    assert other.recommender_plan is not None
    assert other.recommender.evaluate(partition="test", k=5).n_users_scored >= 0
...
def test_method_variants(method: str) -> None:
    ...
    metrics = session.recommender.evaluate(partition="validation", k=5).metrics
...
def test_content_method() -> None:
    ...
    recs = session.recommender.recommend(partition="test", k=3)
    assert recs.n_users > 0
...
def test_implicit_feedback() -> None:
    ...
    assert session.recommender.evaluate(k=5).metrics
...
def test_cold_start_disclosure() -> None:
    ...
    recs = session.recommender.recommend(user_ids=["never_seen_user_xyz"], k=5)
