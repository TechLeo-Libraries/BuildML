...
def test_imitation_and_bandit_smoke(tmp_path: Path) -> None:
    ...
    assert session.rl.evaluate_imitation(partition="validation").metrics["accuracy"] >= 0.0
    session.rl.save_imitation_bundle(tmp_path / "imitation")
    ...
    assert rl.n_arms == 2
    assert session.rl.act(partition="test").n_rows > 0
    ev = session.rl.evaluate(partition="test")
    assert ev.offline is True
    session.rl.save_bundle(tmp_path / "rl")
    ...
    other.rl.load_bundle(tmp_path / "rl", trusted=True)
    assert other.rl.evaluate(partition="validation").offline is True
...
def test_gym_reinforce_smoke(tmp_path: Path) -> None:
    ...
    ev = session.rl.evaluate(n_episodes=5, max_steps=100)
    assert "mean_return" in ev.metrics
    session.rl.save_bundle(tmp_path / "gym_rl")
    other = Session.ingest(frame).set_roles({"a": "feature", "y": "target"})
    other.rl.load_bundle(tmp_path / "gym_rl", trusted=True)
