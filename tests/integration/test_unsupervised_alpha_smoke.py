...
def test_unsupervised_alpha_gate_smoke(tmp_path: Path) -> None:
    ...
    assigned = session.unsupervised.assign(partition="validation")
    ...
    bundle = session.unsupervised.save_bundle(tmp_path / "unsup_bundle")
    ...
    restored.unsupervised.load_bundle(bundle, trusted=True)
    again = restored.unsupervised.assign(partition="validation")
