...
def test_activelearning_alpha_gate_smoke(tmp_path: Path) -> None:
    ...
    ev = session.active_learning.evaluate(partition="validation")
    ...
    bundle = session.active_learning.save_bundle(tmp_path / "al_bundle")
    ...
    restored.active_learning.load_bundle(bundle, trusted=True)
    assert restored.activelearning_plan is not None
    assert restored.activelearning_plan.n_queries_used == 5
    again = restored.active_learning.suggest_query(batch_size=3)
