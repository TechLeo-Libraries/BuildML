...
def render_serve_deployment(
    *,
    ...
    Emits an optional Secret placeholder, a Deployment running ``buildml-serve``
    against a bundle path with readiness/liveness probes on ``/health``, and a
    ClusterIP Service. The default image name matches
    ``deploy/serve/Dockerfile`` (``buildml-serve:local``). Non-loopback bind
    uses an API key from a Secret and never emits
    ...
    api_key_secret_name:
        Secret name providing ``BUILDML_API_KEY``.
    api_key_secret_key:
        Key inside the Secret.
    include_secret:
        When True, also emit a placeholder Secret document (``CHANGE_ME``).
    trusted:
    ...
    **The Service is ClusterIP, reachable only inside the cluster.** External
    ...
    if include_secret:
        secret_doc = (
            ...
            "type: Opaque\n"
            "stringData:\n"
            f'  {api_key_secret_key}: "CHANGE_ME"\n'
            "---\n"
        )
    header = (
        ""
...
def write_serve_deployment(path: str | Path, **kwargs: Any) -> K8sJobRenderResult:
    ...
    return K8sJobRenderResult(
        ...
        limitations=(
            ...
            "Replace Secret api-key before apply; insecure public bind override is off.",
        ),
        meta={
            "name": kwargs.get("name", "buildml-serve"),
            "namespace": kwargs.get("namespace", "default"),
            "image": kwargs.get("image", "buildml-serve:local"),
            "include_secret": kwargs.get("include_secret", True),
        },
    )
