...
# Kubernetes object / field names only. Never a credential value.
_DEFAULT_AUTH_STORE_NAME = "buildml-serve-secrets"
_DEFAULT_AUTH_STORE_FIELD = "api-key"
...
def render_serve_deployment(
    *,
    ...
    kind: str = "pipeline",
    auth_store_name: str = _DEFAULT_AUTH_STORE_NAME,
    auth_store_field: str = _DEFAULT_AUTH_STORE_FIELD,
    ...
    auth_store_name:
        ...
        A name only: this function never writes the key value.
    auth_store_field:
        Field name inside that object. A field name only.
    emit_auth_store_document:
        When True, also emit an empty Opaque Secret (no ``data`` /
        ``stringData``) so operators can ``kubectl apply`` the stub and then
        populate it out of band. Default False: create the store with
    ...
    **The Service is ClusterIP, reachable only inside the cluster.** External
    ...
    store_name = str(auth_store_name)
    store_field = str(auth_store_field)
    _validate_dns_label(store_name, field_name="auth_store_name")
    if not store_field or not str(store_field).replace("-", "").replace("_", "").isalnum():
        raise ValidationError("auth_store_field must be a simple DNS-1123 / data-key label")
    auth_doc = ""
    if emit_auth_store_document:
        auth_doc = (
            "# Generated by buildml.dl.k8s.render_serve_deployment\n"
            "# Honesty: template only: not live multi-cluster / managed IAM.\n"
            "# Image: build from deploy/serve/Dockerfile → buildml-serve:local\n"
            "# Empty store: populate out of band, never commit key values.\n"
            f"#   kubectl create secret generic {store_name} "
            f"--namespace {namespace} "
            f"--from-literal={store_field}=\"$BUILDML_API_KEY\"\n"
            "apiVersion: v1\n"
            "kind: Secret\n"
            "metadata:\n"
            f"  name: {store_name}\n"
    ...
    return (
        ...
        '                echo "BUILDML_API_KEY secret is required '
        '(insecure public bind override is not enabled)" >&2\n'
        "                exit 1\n"
        "              fi\n"
        "              exec buildml-serve \\\n"
...
def write_serve_deployment(
    ...
    *,
    ...
    emit_auth_store_document:
        Emit an empty Opaque Secret stub when True. Passed to the renderer.
    trusted:
