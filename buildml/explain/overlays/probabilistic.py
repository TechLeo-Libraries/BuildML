...
PROBABILISTIC_PLAN = Prerequisite(
    ...
    check_hint="session.probabilistic.plan is not None.",
)
...
_OPERATIONS: tuple[OperationSpec, ...] = (
    _operation(
        ...
        outputs=("ProbabilisticFitResult; session.probabilistic.plan attached.",),
        ...
        state_changes=(
            "Stores session.probabilistic.plan and fit result; clears prior eval/predict/interval slots.",
        ),
        result_reading=(
            "Read n_fit_rows, n_conformal_calib_rows, conformal_quantile, disclosures.",
        ),
        next_steps=(
            "session.probabilistic.predict_interval → session.probabilistic.evaluate; optionally session.probabilistic.save_bundle.",
        ),
        concepts=(
    ...
    _operation(
        ...
        ordering=("After session.probabilistic.fit or session.probabilistic.load_bundle.",),
        alternatives=(
        ...
        state_changes=("Stores session.probabilistic.eval_result.",),
        result_reading=(
            "Read metrics (nll, interval_coverage / set_coverage, brier/ece), disclosures.",
        ),
        next_steps=("session.probabilistic.predict_interval; session.probabilistic.save_bundle.",),
        concepts=(
            "probabilistic-uncertainty",
            "probabilistic-split-conformal",
    ...
    _operation(
        ...
        prerequisites=(DATASET, PROBABILISTIC_PLAN),
        ordering=("After session.probabilistic.fit.",),
        alternatives=("session.probabilistic.predict_interval when you need bands/sets.",),
        ...
        anti_patterns=("Treating std as conformal coverage without session.probabilistic.predict_interval.",),
        state_changes=("Stores session.probabilistic.predict_result.",),
        result_reading=("Read predictions / std / probabilities and disclosures.",),
        next_steps=("session.probabilistic.predict_interval or session.probabilistic.evaluate.",),
    ...
    _operation(
        ...
        prerequisites=(DATASET, PROBABILISTIC_PLAN),
        ordering=("After session.probabilistic.fit.",),
        alternatives=("session.probabilistic.evaluate for coverage metrics.",),
        ...
        state_changes=("Stores session.probabilistic.interval_result.",),
        result_reading=("Read lower/upper or prediction_sets, method, disclosures.",),
        next_steps=("session.probabilistic.evaluate; session.probabilistic.save_bundle.",),
        concepts=(
            "probabilistic-split-conformal",
            "probabilistic-uncertainty",
        ),
    ...
    _operation(
        ...
        ordering=("After a successful session.probabilistic.fit.",),
        ...
        state_changes=("History records session.probabilistic.save_bundle.",),
        result_reading=("Confirm meta.json format buildml.probabilistic_bundle.v1.",),
        next_steps=("session.probabilistic.load_bundle in another Session.",),
    ...
    _operation(
        ...
        outputs=("Session with session.probabilistic.plan attached.",),
        prerequisites=(DATASET,),
        ordering=("After ingest/roles/split aligned with the plan feature contract.",),
        alternatives=("session.probabilistic.fit to learn a new plan.",),
        ...
        state_changes=(
            "Sets session.probabilistic.plan; clears fit/eval/predict/interval result slots.",
        ),
        result_reading=("Inspect session.probabilistic.plan.to_dict().",),
        next_steps=("session.probabilistic.evaluate / session.probabilistic.predict_interval.",),
        concepts=("probabilistic-bundle-boundary",),
    ),
)
