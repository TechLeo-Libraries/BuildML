...
CLUSTER_PLAN = Prerequisite(
    ...
    check_hint="session.unsupervised.plan is not None.",
)
...
_OPERATIONS: tuple[OperationSpec, ...] = (
    _operation(
        ...
        state_changes=(
            "Stores session.unsupervised.plan and session.unsupervised.fit_result; clears prior assign/eval slots.",
        ),
        result_reading=(
            "Read method, n_clusters, cluster_sizes, assign_strategy, used_reduce_components, disclosures.",
        ),
        next_steps=("session.unsupervised.assign and/or session.unsupervised.evaluate; optionally session.unsupervised.save_bundle.",),
        concepts=(
    ...
    _operation(
        ...
        ordering=("After session.unsupervised.fit or session.unsupervised.load_bundle.",),
        alternatives=("session.unsupervised.evaluate when you need metrics rather than raw labels.",),
        ...
        state_changes=("Stores session.unsupervised.assign_result; may add a feature-role label column.",),
        result_reading=("Read n_rows, n_noise, assign_strategy, and disclosures.",),
        next_steps=("session.unsupervised.evaluate; or use attached labels cautiously downstream.",),
        concepts=(
            "unsupervised-train-fit-holdout-assign",
            "cluster-validity-not-truth",
            "leakage-boundary",
    ...
    _operation(
        ...
        prerequisites=(DATASET, CLUSTER_PLAN),
        ordering=("After session.unsupervised.fit; prefer validation/test over train for claims.",),
        alternatives=("session.unsupervised.assign for labels without metrics.",),
        ...
        state_changes=("Stores session.unsupervised.eval_result.",),
        result_reading=(
            "Read metrics, external_metrics, n_clusters_observed, disclosures, recommendations.",
        ),
        next_steps=("session.unsupervised.save_bundle; or revise features/k and refit on train.",),
        concepts=(
            "cluster-validity-not-truth",
            "unsupervised-train-fit-holdout-assign",
    ...
    _operation(
        ...
        ordering=("After session.unsupervised.fit (and usually after session.unsupervised.evaluate).",),
        ...
        next_steps=("session.unsupervised.load_bundle on a fresh Session when needed.",),
    ...
    _operation(
        ...
        outputs=("Session with session.unsupervised.plan attached.",),
        prerequisites=(),
        ordering=("Before session.unsupervised.assign / session.unsupervised.evaluate on a restored plan.",),
        alternatives=("Refit with session.unsupervised.fit when the feature space changed.",),
        ...
        anti_patterns=("Loading a RAG/Torch bundle path into session.unsupervised.load_bundle.",),
        state_changes=("Stores session.unsupervised.plan; clears prior fit/assign/eval result slots.",),
        result_reading=("Check method, columns, assign_strategy from plan.to_dict().",),
        next_steps=("session.unsupervised.assign or session.unsupervised.evaluate once features are available.",),
        concepts=("unsupervised-bundle-boundary", "unsupervised-train-fit-holdout-assign"),
    ),
)
