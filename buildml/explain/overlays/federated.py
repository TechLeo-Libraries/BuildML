...
FEDERATED_PLAN = Prerequisite(
    ...
    check_hint="session.federated.plan is not None.",
)
...
_OPERATIONS: tuple[OperationSpec, ...] = (
    _operation(
        ...
        outputs=("FederatedFitResult; session.federated.plan attached.",),
        prerequisites=(DATASET, ROLES, SPLIT),
        ordering=("After ingest → set_roles → split → optional scale/reduce.",),
        alternatives=(
            "session.online.fit for single-stream partial_fit; fit for centralized pooling.",
        ),
        rationale=(
        ...
        state_changes=(
            "Stores session.federated.plan and fit result; clears prior eval/predict slots.",
        ),
        result_reading=(
            "Read n_clients, round_history, final_train_metric, disclosures.",
        ),
        next_steps=(
            "session.federated.evaluate → session.federated.predict; optionally session.federated.save_bundle.",
        ),
        concepts=(
    ...
    _operation(
        ...
        ordering=("After session.federated.fit or session.federated.load_bundle.",),
        alternatives=("session.federated.predict when you only need labels.",),
        ...
        leakage=(
            "Using session.federated.evaluate metrics to choose which holdout clients to retrain on.",
        ),
        anti_patterns=(
            "Reporting per-client train metrics as privacy-preserving FL results.",
        ),
        state_changes=("Stores session.federated.eval_result.",),
        result_reading=(
            "Read metrics, per_client_metrics, n_clients_evaluated, disclosures.",
        ),
        next_steps=("session.federated.save_bundle; or session.federated.predict.",),
        concepts=(
            "federated-simulation",
            "federated-fedavg",
    ...
    _operation(
        ...
        prerequisites=(DATASET, FEDERATED_PLAN),
        ordering=("After session.federated.fit / session.federated.evaluate.",),
        alternatives=("session.federated.evaluate when you also need metrics.",),
        ...
        state_changes=("Stores session.federated.predict_result.",),
        result_reading=("Read n_rows / n_predictions and disclosures.",),
        next_steps=("session.federated.evaluate or session.federated.save_bundle.",),
        concepts=("federated-simulation",),
    ),
    _operation(
        "export_round_history",
        OperationKind.PERSIST,
        "Write federated round metrics history to a JSON file.",
        "Export per-round client/server metrics after session.federated.fit for audit or plotting.",
        "Federated round-history export.",
        (
            "Require an attached federated fit result with round history.",
            "Serialize rounds to JSON at the given path.",
        ),
        parameters=(_p("path", "str | Path", "Destination JSON file.", required=True),),
        inputs=("Active federated fit result with round history.",),
        outputs=("Path to the written JSON file.",),
        prerequisites=(FEDERATED_PLAN,),
        ordering=("After session.federated.fit (and optionally session.federated.evaluate).",),
        alternatives=("Reading session.federated.fit_result.to_dict() in-process.",),
        rationale=("Use when round curves must leave the Session for reports or CI artifacts.",),
        assumptions=("Destination directory is writable.",),
        failures=("No federated fit result / empty round history.",),
        leakage=("Export does not embed raw client datasets.",),
        anti_patterns=("Treating exported metrics as proof of cross-silo privacy guarantees.",),
        state_changes=("History records session.federated.export_round_history.",),
        result_reading=("Open the JSON and confirm round count matches n_rounds.",),
        next_steps=("session.federated.save_bundle when the global model must travel too.",),
        concepts=("federated-simulation",),
    ),
    _operation(
        ...
        prerequisites=(FEDERATED_PLAN,),
        ordering=("After a successful session.federated.fit.",),
        ...
        state_changes=("History records session.federated.save_bundle.",),
        result_reading=("Confirm meta.json format buildml.federated_bundle.v1.",),
        next_steps=("session.federated.load_bundle in another Session.",),
    ...
    _operation(
        ...
        outputs=("Session with session.federated.plan attached.",),
        prerequisites=(DATASET,),
        ordering=("After ingest/roles/split aligned with the plan feature contract.",),
        alternatives=("session.federated.fit to learn a new plan.",),
        ...
        state_changes=(
            "Sets session.federated.plan; clears fit/eval/predict result slots.",
        ),
        result_reading=("Inspect session.federated.plan.to_dict().",),
        next_steps=("session.federated.evaluate / session.federated.predict.",),
        concepts=("federated-bundle-boundary",),
    ),
)
